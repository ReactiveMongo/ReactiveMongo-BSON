...
private[bson] sealed trait LowPriorityHandlerConverters2
   ...
   * }}}
   */
  implicit final def toDocumentWriter[T](w: LegacyDocWriter[T]): BSONDocumentWriter[T] = BSONDocumentWriter[T] { t => ValueConverters.toDocument(w write t) }
   ...
   * }}}
   */
  implicit final def toDocumentReader[T](r: LegacyDocReader[T]): BSONDocumentReader[T] = BSONDocumentReader[T] { bson =>
