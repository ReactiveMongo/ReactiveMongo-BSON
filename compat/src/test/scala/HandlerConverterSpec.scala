  ...
  "Converters" should {
    ...
    "from legacy document" >> {
      "in writer" in {
        ...
        def bw: BSONDocumentWriter[Double] = lw

        toWriter(lw).writeTry(1.0D) must beSuccessfulTry(doc) and {
          bw.writeTry(1.1D) must beSuccessfulTry(doc)
        }
      }

      "in reader" in {
        implicit val lr = LegacyDocReader[Float](_ => 1.2F)
        def br: BSONDocumentReader[Float] = lr

        toDocumentReader(lr).readTry(
          BSONDocument("ok" -> 1)) must beSuccessfulTry(1.2F) and {
            br.readTry(BSONDocument("ok" -> 2)) must beSuccessfulTry(1.2F)
          }
      }
